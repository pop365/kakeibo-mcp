...
def 仕訳記録(
    ...
    引数:
      日付     : YYYY-MM-DD 形式（例: 2026-06-20）
      借方科目 : 借方の科目名（科目一覧で確認すること）
      貸方科目 : 貸方の科目名（科目一覧で確認すること）
      金額     : 円単位の整数（例: 3000）
      摘要     : 取引の説明（省略可）

    よくある仕訳パターン:
      スーパーで現金払い → 借方:食費 / 貸方:現金
      給与入金          → 借方:普通預金 / 貸方:給与収入
      カード払い        → 借方:食費 / 貸方:クレジットカード
      カード引落し      → 借方:クレジットカード / 貸方:普通預金
    """
    try:
        entry = add_journal_entry(日付, 借方科目, 貸方科目, 金額, 摘要)
        return (
            f"✅ 仕訳を記録しました\n"
            f"  ID  : {entry['id']}\n"
            f"  日付: {entry['date']}\n"
...
def 仕訳一覧(年月: str = "") -> str:
    ...
    引数:
      年月: YYYY-MM 形式で絞り込み（例: 2026-06）。省略すると全件表示。
    """
    entries = get_journal_entries(年月 or None)
    if not entries:
        label = f"「{年月}」" if 年月 else "全期間"
...
@mcp.tool()
def 仕訳削除(仕訳ID: str) -> str:
    """
    指定したIDの仕訳を削除します。
    仕訳IDは「仕訳一覧」ツールで確認できます。
    """
    if delete_journal_entry(仕訳ID):
        return f"✅ 仕訳 [{仕訳ID}] を削除しました"
    return f"❌ 仕訳 [{仕訳ID}] が見つかりません"


@mcp.tool()
def 残高試算表(年月: str = "") -> str:
    """
    残高試算表を表示します。各科目の借方・貸方合計と残高を確認できます。

    引数:
      年月: YYYY-MM 形式で絞り込み（例: 2026-06）。省略すると全期間。
    """
    balances = get_trial_balance(年月 or None)
    if not balances:
        return "データがありません"
...
def 月次収支(年月: str) -> str:
    ...
    引数:
      年月: YYYY-MM 形式（例: 2026-06）
    """
    summary = get_monthly_summary(年月)
...
@mcp.tool()
def 科目追加(科目名: str, 種別: str) -> str:
    """
    新しい勘定科目を追加します。

    引数:
      科目名: 追加する科目の名前
      種別  : 「資産」「負債」「資本」「収益」「費用」のいずれか
    """
    try:
        acc = add_account(科目名, 種別)
